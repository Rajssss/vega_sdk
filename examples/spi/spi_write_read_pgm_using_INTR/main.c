...
/** @fn spi_intr_enable
 ...
 @param[Out] No output parameter. 
*/
void  spi_intr_enable(UC spi_number,UC rx_intr, UC tx_intr)
{
	US cword = 0;
	cword = SPIreg(spi_number).Control.hword; 
	if(rx_intr == RXINTR_EN)
		cword |= (1 << 6);	
	else if(rx_intr == RXINTR_DIS)
		cword &= ~(1 << 6);	

	if(tx_intr == TXINTR_EN)
		cword |= (1 << 7);	
	else if(tx_intr == TXINTR_DIS)
		cword &= ~(1 << 7);	

	SPIreg(spi_number).Control.hword = cword; 
}

/** @fn spi0_intr_handler
 @brief Program eeprom.
 @details Write, read and compare eeprom locations.
 @warning 
 @param[in] No input parameter. 
 @param[Out] No output parameter. 
*/
void spi0_intr_handler(void){

    UC intr_status=spi_intr_status(0);

    if(intr_status == SPI_RX_INTR) {
    ///add code to handle spi rx
    }

    else if(intr_status == SPI_TX_INTR) {
        //add code to handle spi tx
    }
}
...
void main ()
{
	spi_register_isr(0, spi0_intr_handler);
    	spi_intr_enable(0,RXINTR_EN,TXINTR_EN);
